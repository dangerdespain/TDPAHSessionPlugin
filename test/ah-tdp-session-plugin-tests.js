...
describe('actionhero plugin ah-tdp-session-plugin tests', function()
{
  ...
  {
      api.specHelper.runAction('ah-tdp-session-plugin/saveSession', conn, function(response, connection)
      {
  ...
  {
      api.specHelper.runAction('ah-tdp-session-plugin/loadSession', conn, function(response, connection)
      {
        assert.deepEqual(response.sessionData, testObj);
        return done();
      });
